...
function Home() {
  ...
  return (
    <main className="bg-white">
      ...
      <section className="py-14 sm:py-16"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-2xl font-bold text-gray-900 sm:text-3xl">Featured Events</h2><p className="mt-2 text-gray-600">Halkan waxaad ka heli doontaa munaasabadaha la soo bandhigay.</p></div><Link to="/events" className="font-semibold text-blue-600 hover:text-blue-700">View All Events</Link></div><div className="mt-8">{loading ? <PageState message="Loading events..." /> : error ? <PageState tone="error" message={error} /> : events.length ? <EventGrid events={events} /> : <PageState title="No featured events" message="Dhacdooyin la heli karo ma jiraan hadda." />}</div></div></section>
      <section id="how-it-works" className="bg-gray-50 py-14 sm:py-16"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="max-w-2xl"><h2 className="text-2xl font-bold text-gray-900 sm:text-3xl">How It Works</h2><p className="mt-2 text-gray-600">Afar tallaabo oo fudud ayaa kuu dhexeeya munaasabadda iyo tigidhkaaga.</p></div><div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{steps.map((step, index) => { const Icon = step.icon; return <article key={step.title} className="border-t-2 border-blue-600 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><Icon className="text-2xl text-blue-600" aria-hidden="true" /><span className="text-sm font-semibold text-gray-400">0{index + 1}</span></div><h3 className="mt-5 font-semibold text-gray-900">{step.title}</h3><p className="mt-2 text-sm leading-6 text-gray-600">{step.description}</p></article> })}</div></div></section>
